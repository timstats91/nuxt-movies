    ...
    <div class="container movies">
      ...
      <div v-if="searchInput !== ''" id="movie-grid" class="movies-grid">
        <div class="movie" v-for="(movie, index) in searchedMovies" :key="index">
          <div class="movie-img">
            <img :src="`https://tmdb.org/t/p/w500/${movie.poster_path}`" alt="">
  ...
  methods: {
    async getMovies() {
      const data = axios.get(
        'https://api.themoviedb.org/3/movie/now_playing?api_key=d4be5df613286799989a0a84c536f851&language=en-US&page=1'
      )
    ...
    async searchMovies () {
      const data = axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=d4be5df613286799989a0a84c536f851&language=en-US&page=1&query=${this.searchInput}`
      )
      const result = await data
      result.data.results.forEach((movie) => {
